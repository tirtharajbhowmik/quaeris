...
- **Selection (AS BUILT — supersedes the original alias idea):** `scripts/select-brand.mjs` reads `process.env.BRAND` (default `veris`) and writes `src/brand-active/{config.ts,fonts.ts,tokens.css}` as re-exports of `src/brands/<brand>/*`. The directory is gitignored; `postinstall` and every dev/build/typecheck script regenerate it. There is NO `@brand` bundler alias and no `next.config.ts` involvement.
- **Imports (AS BUILT):** components keep importing `{ brand } from "@/config/brand.config"` — that file is a shim re-exporting `./types` (all interfaces) plus `@/brand-active/config` (values). `layout.tsx` imports `@/brand-active/tokens.css`, `fontClasses` from `@/brand-active/fonts` (applied to `<html>` — :root token declarations need the font variables there), and derives metadata from the brand config.
- **Shared contract (AS BUILT):** `src/config/types.ts` holds all shared interfaces (incl. `BrandMeta`); there is no umbrella `BrandConfig` interface — per-field `satisfies` clauses in each brand config plus the dual-brand `tsc`/build gate provide shape safety. Brand config modules export VALUES only (`brand`, `images`); a type re-export there would silently shadow shim names (ambiguous star-exports are dropped, not errored).
